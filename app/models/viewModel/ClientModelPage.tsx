...
import { Card, Spinner, Table, Button, Badge, Alert, Toast } from "flowbite-react";
...
export default function ClientModelPage() {
  ...
  const [generatingReports, setGeneratingReports] = useState<{ [key: number]: boolean }>({});
  const [notification, setNotification] = useState<{ type: 'success' | 'failure'; message: string } | null>(null);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (statusData.status === 'SUCCESS') {
          ...
          setNotification({ type: 'success', message: 'Report generated successfully! Redirecting...' });
  ...
  const handleGenerateReport = async (modelId: number) => {
    ...
    try {
      ...
      if (data.success && data.task_id) {
        setNotification({ type: 'success', message: 'Report generation started... Please wait.' });
    ...
    <div className="p-4 sm:p-6 lg:p-8">
        {notification && (
            <Toast className="fixed top-5 right-5 z-50">
                {notification.type === 'success' ? <HiCheck className="h-5 w-5 text-cyan-600" /> : <HiExclamation className="h-5 w-5 text-red-500" />}
